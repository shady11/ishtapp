...
class BusynessController extends Controller
{
    public function index()
    {
        ...
        return view('admin.busynesses.index', compact('title'));
    }
    ...
    public function create()
    {
        ...
        return view('admin.busynesses.create', compact('busyness', 'title'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('busynesses.index');
    }

    public function edit(Busyness $busyness)
    {
        $title = 'Виды занятости';
        return view('admin.busynesses.edit', compact('busyness', 'title'));
    }
    ...
    public function api(Request $request){
        ...
        $resultPaginated = Busyness::orderBy('name', 'asc')->orderBy('name', 'asc');
        ...
        foreach ($resultPaginated as $key => $row) {
            ...
            $row->actions = '
                <a href="'.route('busynesses.edit', $row).'" class="btn btn-sm btn-clean btn-icon mr-2" title="Редактировать">
                ...
                <a href="'.route('busynesses.delete', $row).'" class="btn btn-sm btn-clean btn-icon" title="Удалить">
                    <span class="svg-icon svg-icon-md">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                            <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
