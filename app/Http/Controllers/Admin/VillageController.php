...
{
    public function create(Region $region, District $district)
    {
        $village = new Village();
        $title = 'Села - '.$region->nameRu.' - '.$district->nameRu;

        return view('admin.villages.create', compact('village', 'district', 'region', 'title'));
    }

    public function store(Request $request, Region $region, District $district)
    {
        ...
        $village->region = $region->id;
        $village->district = $district->id;
        $village->save();

        return redirect()->route('regions.districts.show', [$region, $district]);
    }

    public function edit(Region $region, District $district, Village $village)
    {
        $title = 'Села -'.$district->nameRu;
        return view('admin.villages.edit', compact('village', 'district', 'region', 'title'));
    }

    public function update(Request $request, Region $region, District $district, Village $village)
    {
        $this->validate($request, [
            'nameKg' => ['required'],
            'nameRu' => ['required'],
    ...
    public function destroy(Region $region, District $district, Village $village)
    {
        $village->delete();
        return redirect()->route('regions.districts.show', [$region, $district]);
    }

    public function api(Request $request, Region $region, District $district){
        ...
        $resultPaginated = Village::where('district', $district->id);

        if($sort && $sort['field'] != 'order'){
            $resultPaginated = $resultPaginated->orderBy($sort['field'], $sort['sort']);
        } else {
            $resultPaginated = $resultPaginated->orderBy('nameKg', 'asc');
        }
        ...
        foreach ($resultPaginated as $key => $row) {
            ...
            $row->actions = '
                <a href="'.route('regions.districts.villages.edit', [$region, $district, $row]).'" class="btn btn-sm btn-clean btn-icon mr-2" title="Редактировать">
                ...
                <a href="'.route('regions.districts.villages.delete', [$region, $district, $row]).'" class="btn btn-sm btn-clean btn-icon" title="Удалить">
                    <span class="svg-icon svg-icon-md">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                            <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
