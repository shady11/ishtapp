...
class VacancyTypeController extends Controller
{
    ...
    public function api(Request $request)
    {
        ...
        $resultPaginated = VacancyType::orderBy('name', 'asc')->orderBy('name', 'asc');
        ...
        foreach ($resultPaginated as $key => $row) {
            ...
            $row->actions = '
                <a href="'.route('vacancy_types.edit', $row).'" class="btn btn-sm btn-clean btn-icon mr-2" title="Редактировать">
                ...
                <a href="'.route('vacancy_types.delete', $row).'" class="btn btn-sm btn-clean btn-icon" title="Удалить">
                    <span class="svg-icon svg-icon-md">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                            <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
