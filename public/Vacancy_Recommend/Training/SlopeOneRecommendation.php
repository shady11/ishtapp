 ...
 * 
 */
class SlopeOneRecommendation extends Base
{
    ...
    private function update($table, $user)
    {
        ...
        foreach($this->product as $item){
            $this->myProduct[$item[self::USER_ID]][$item[self::vacancy_id]] = $item[self::SCORE];
        }
        foreach($this->other as $item){ 
            $this->factor[$item[self::USER_ID]][$item[self::vacancy_id]] = $item[self::SCORE]; 
        }
    }
